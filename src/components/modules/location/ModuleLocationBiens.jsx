...
export default function ModuleLocationBiens({ biens, contactMap, search }) {
  const filtered = biens.filter(b =>
    [b.titre, b.adresse].some(v => v?.toLowerCase().includes(search.toLowerCase()))
  );

  if (!filtered.length) return <Empty label="Aucun bien en location" />;
      ...
      <table className="w-full text-sm">
        <thead className="bg-secondary/30">
          <tr>
            ...
            <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Loyer / mois</th>
            ...
            <tr key={b.id} className="hover:bg-secondary/10 transition-colors">
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <div className="p-1.5 bg-emerald-50 rounded-lg"><Home className="w-3.5 h-3.5 text-emerald-500" /></div>
                  <div>
